...
if not API_KEY:
    ...
    # For now, use a placeholder - user must set the environment variable
    API_KEY = "PLACEHOLDER_API_KEY"

api_client = SpoonacularAPI(API_KEY)

def cargar_local(filename: str):
    """Función de utilidad para leer un JSON local si ya hiciste la petición antes."""
    try:
...
def generar_nombre_archivo(recipe_name: str) -> str:
    """Genera un nombre de archivo con formato recipename_timestamp."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"{recipe_name}_{timestamp}.json"

def asegurar_directorio_response():
    """Asegura que el directorio responsesapi exista dentro de datalake_bronze/API use/."""
    response_dir = os.path.join("..", "..", "datalake_bronze", "API use", "responsesapi")
...
def main():
    ...
    try:
        ...
        print("Collecting raw recipe data by ingredients (simplified version for easier collection)...\n")
            ...
            for dietary_combo in dietary_combinations:
                try:
                    ...
                    data = collect_recipes_by_ingredient(
                        ...
                        number=20,  # Collect 20 recipes per ingredient for substantial dataset
        ...
        print("This dataset provides substantial recipe data for sentiment analysis!")
